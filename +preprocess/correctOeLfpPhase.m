function [data] = correctOeLfpPhase(x, sampleRate, cutoff)
...
% Inputs: 
%   x      [n x 1]  raw data
%   Fs     [1 x 1]  sampling rate 
%   cutoff [1 x 1]  the cutoff frequency (Hz) used in the recording 
%                    (currently only 0.1 and 1.0 values are supported) 
%
% Outputs:
%  data    [n x 1]  the continuous trace after low frequency correction
...
x = x - mean(x);
N = 2^nextpow2(min(numel(x), sampleRate*500)); % ~500s chunks

freqX  = sampleRate * (1:N/2-1)/N; % frequency of each FFT component (to be computed below)
